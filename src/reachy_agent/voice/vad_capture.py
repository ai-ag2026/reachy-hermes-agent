...
class VAD:
    ...
    def __call__(self, audio_sample: NDArray[np.float32], sample_rate: int = SAMPLE_RATE) -> NDArray[np.float32]:
        ...
        if audio_sample.shape[-1] != num_samples:
            raise ValueError(
                f"Provided number of samples is {audio_sample.shape[-1]} (Supported: 256 for 8kHz, 512 for 16kHz)"
            )
...
class VADSpeechCapture:
    ...
    async def _process_vad(self, chunk: NDArray[np.int16]) -> bool:
        ...
        confidence = await asyncio.get_event_loop().run_in_executor(None, self.vad, audio_float, self.sample_rate)

        conf = float(confidence)
        self._last_confidence = conf
        return conf > self.vad_threshold

    def _manage_pre_activation_buffer(self, chunk: NDArray[np.int16], vad_confidence: bool) -> None:
        """
    ...
    def _process_activated_audio(self, chunk: NDArray[np.int16], vad_confidence: bool) -> bool:
        """
