...
Supports three backends (configured via ``engine`` parameter):
...
- ``remote_http`` — OpenAI-compatible HTTP transcription endpoint
  (``POST <base_url>/audio/transcriptions`` with multipart file + model).
  Target for ARM hosts (CM4) offloading STT to a GPU server.
"""
...
class STTEngine:
    ...
    def __init__(
        ...
        # Remote HTTP config (used when engine == "remote_http")
        ...
        elif engine == "remote_http":
            self._init_remote_http()
        else:
    ...
    def _init_faster_whisper(self, model_size: str, device: str, compute_type: Optional[str]) -> None:
        ...
        logger.info(f"Initializing Faster Whisper: {model_size} on {device} ({self.compute_type})")
    ...
    def _init_remote_http(self) -> None:
        if not self._remote_url:
            raise ValueError("remote_http STT engine requires remote_url in config")
        if not self._remote_model:
            raise ValueError("remote_http STT engine requires remote_model in config")
        # Normalize: base URL may be given with or without trailing /audio/transcriptions.
        url = self._remote_url.rstrip("/")
        if not url.endswith("/audio/transcriptions"):
            url = url + "/audio/transcriptions"
        self._remote_endpoint = url
        logger.info(f"Remote HTTP STT: model={self._remote_model!r} endpoint={self._remote_endpoint}")

    def _init_parakeet(self, model_path: Optional[str], gpu_device: int) -> None:
        ...
        if not os.path.isdir(model_path):
            logger.info(f"Parakeet model not found at {model_path}, downloading …")
            from huggingface_hub import snapshot_download

            os.makedirs(model_path, exist_ok=True)
            snapshot_download(
                "istupakov/parakeet-tdt-0.6b-v2",
        ...
        logger.info(f"Initializing Parakeet-TDT-0.6b-v2 on GPU {gpu_device} from {model_path}")
    ...
    def transcribe(self, audio_wav: bytes) -> STTResult:
        ...
        try:
            ...
            if self.engine == "remote_http":
                return self._transcribe_remote_http(audio_wav)
    ...
    def _transcribe_parakeet(self, audio_wav: bytes) -> STTResult:
        ...
        # Parakeet doesn't provide logprob / no_speech_prob. avg_logprob=1.0 is above
        # min_speech_confidence (0.6) so the confidence gate passes (0.0 would have made
        # is_speech always False). NOTE: is_speech from this backend is still only a rough
        # length/compression heuristic — the live front-end does NOT gate on it (it uses
        # empty-text -> None); treat is_speech as advisory, not authoritative.
        return self._build_result(
    ...
    def _transcribe_remote_http(self, audio_wav: bytes) -> STTResult:
        import requests

        duration = self._get_wav_duration(audio_wav)

        files = {"file": ("audio.wav", audio_wav, "audio/wav")}
        data = {"model": self._remote_model}
        if self.language:
            data["language"] = self.language
        headers = {}
        if self._remote_auth_token:
            headers["Authorization"] = f"Bearer {self._remote_auth_token}"
        ...
        try:
            ...
            raise RuntimeError(f"Remote STT request failed: status={response.status_code} body={body!r}") from e

        payload = response.json()
        text = (payload.get("text") or "").strip()

        # Remote endpoint doesn't return logprob / no_speech_prob. avg_logprob=1.0 clears the
        # confidence gate (0.0 would force is_speech False). As with parakeet, is_speech here is
        # only advisory — the live front-end gates on empty-text -> None, not on this field.
        return self._build_result(
            text,
            duration,
            self.language or "en",
    ...
    def _is_speech(
        ...
        compression_ok = self.min_compression_ratio < compression_ratio < self.max_compression_ratio
        ...
        if not is_speech:
            ...
            if not confidence_ok:
                reasons.append(f"low confidence (avg_logprob={avg_logprob:.2f} < {self.min_speech_confidence})")
            if not no_speech_ok:
                reasons.append(f"high no_speech_prob ({no_speech_prob:.2f} > {self.max_no_speech_prob})")
